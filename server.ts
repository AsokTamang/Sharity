...
app.prepare().then(()=>{
  ...
  io.on('connection',(socket:any)=>{
    ...
    socket.on('join-room',async(data:{roomID:string})=>{    //then we let the user to join the private chat room using the roomID
      const {roomID}=data;  //destructering the roomID
      socket.join(roomID);   //joining with the roomID
      const lastMessages=await messageModal.find({roomId:roomID}).sort({timeStamp:1}).populate('sender','email contact');   //fetching the saved messages based on the roomID so we are using find method not findById
      socket.emit('last messages',lastMessages)   //then we emit that last or history messages
    });

    socket.on('chat messages',async(data:{msg:messgaeDataType,roomID:string})=>{   //then if there is any messages in the room then 
      ...
      io.to(roomID).emit('chat messages',{...msg, sender:{   //then we emit that new messages in that specific roomID's chat room
