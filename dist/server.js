...
app.prepare().then(() => {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("join-room", async (data) => {
            //we are passing the roomID as an object here
            ...
            const existingRoom = await roomidModal.findOne({ roomid: roomID });
            if (!existingRoom) {
                const savedRoomId = new roomidModal({
                    //we are storing the roomId in our different roomid modal which will have the collection of the roomids
                    roomid: roomID,
                });
                await savedRoomId.save();
            } //then we save that roomid
        });
        socket.on("chat messages", async (data) => {
            ...
            io.to(roomID).emit("chat messages", Object.assign(Object.assign({}, msg), { sender: {
