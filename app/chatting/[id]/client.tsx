...
export default  function ChattingClientpage({itemID,itemOwnerID}:{itemID:string,itemOwnerID:string}){
  ...
  React.useEffect(()=>{
    ...
    socket.emit('join-room',{roomID:{roomID:`${userID}_${itemOwnerID}_${itemID}`||`${itemOwnerID}_${userID}_${itemID}`}});   //here we are passing the join-room event to our backend server using the owner's id as the roomID.
  ...
  const  handleSend=()=>{
    if (message.trim()===""|| !socketRef.current) return;  //here we are using trim to exclude any whitespaces or empty spaces at the beginning and at the end of a string or character and even after this if the message is empty then we return the function
      ...
      socketRef.current.emit('chat messages',{msg,roomID:`${userID}_${itemOwnerID}_${itemID}`||`${itemOwnerID}_${userID}_${itemID}`});  // then we pass an event called chat messsages to our backend
 ...
 return (
  <div className="flex flex-col h-5/6 p-4 bg-gray-100">
    {/* Chat messages */}
